...
echo "up /etc/openvpn/scripts/update-systemd-resolved" >> /vpn.modified.conf
echo "down /etc/openvpn/scripts/update-systemd-resolved" >> /vpn.modified.conf
...
exec /openvpn --config /vpn.modified.conf \
              ...
              --down-pre \
              --iproute /usr/local/bin/routedev \
